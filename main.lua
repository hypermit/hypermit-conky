...
function conky_main(  )
	...
	-- value text
	--local cputemp = conky_parse("${execi 10 sensors | grep 'Core 0' | awk {'print $3'}}");
	local cputemp = conky_parse("${execi 10 sensors |grep temp1 |awk '{print$2}' |cut -c2-5}");
	if text == "U" then
		text=""
		-- image
		local ir = cairo_create(cs);
	...
	cairo_text_extents(cr, text, extents)
	cairo_move_to(cr, item_centerx - extents.width/3, item_centery + item_radius + item_font_size + 8);
	cairo_show_text(cr, cputemp);
